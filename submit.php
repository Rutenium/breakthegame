...
?>
		...
		<h1>Type:</hi>
		<input class="round" id="base" type="text" name="base" value="Choose a game first!" readonly <?php if(isset($_POST['game'])){echo 'style="display:none;"';}?>>
		<?php
		$reponse = $bdd->query('SELECT id,types FROM games ORDER BY id') or die(print_r($bdd->errorInfo()));
		while($donnees = $reponse->fetch())
		{
		?>
		<select class="round" id="type_<?php echo $donnees['id'] ?>" name="type"<?php if(isset($_POST['game']) AND ($_POST['game']==$donnees['id']) ) {echo 'style="display:block;"';}else{echo 'style="display:none;"';}?>>
